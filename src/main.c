...
void process_into_objects()
{
	...
	for(int i = 0; i<linesValidLength; i++)
	{
		...
		//iterate through future valid lines to find recipes
		int x = i+1;
...
//executes the recipes of a single target
void execute_target(char *targetname)
{
	//searches for the target for us. before this is called, we have already
	//checked that the target exists in our error cases in main()
	int x = 0;
	node r = root;
	while(x == 0 && r != NULL)
	{
	...
	//two seperate delimiters to get the full recipe and the path respectively
	char *delimiter = "\t\n";
	char *delimiter2 = "\t \n";
	pid_t cpid;
...
int main(int argc, char *argv[])
{
	...
	if (argc == 2)
	{
		...
		{
			//don't need to worry for a valid target since we just pick the first one
			//in the file
	...
	else if (argc == 3)
	{
		...
		else if (process_file(argv[1]) == 0)
		{
		  ...
		  while(x == 0 && r != NULL)
		  {
			...
			//makes sure that the target exists and that the correct
			//order for flags was supplied
			if(x==0)
			{
